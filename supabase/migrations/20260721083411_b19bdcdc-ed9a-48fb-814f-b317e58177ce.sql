...
CREATE POLICY "public read feed_ingredients" ON public.feed_ingredients FOR SELECT USING (true);
CREATE POLICY "admin write feed_ingredients" ON public.feed_ingredients FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
